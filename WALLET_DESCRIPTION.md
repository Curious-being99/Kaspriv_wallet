...
*   **Military-Grade Security:**
    *   **Argon2id Key Derivation:** Utilizes the RFC 9106 recommended memory-hard function (64 MiB memory, 4 iterations, 1 parallel lane) to derive encryption keys from your password, making brute-force attacks computationally prohibitive.
    *   **AES-256-GCM Encryption:** All sensitive wallet data is encrypted using authenticated AES-256-GCM via the browser's native Web Crypto API. Nonces/IVs are randomly generated and strictly single-use.
    *   **Fragmented Storage:** The mnemonic seed is split, cryptographically shuffled, and encrypted in isolated fragments to prevent straightforward extraction from local storage.
    *   **In-Memory Sanitization:** Sensitive data buffers (keys, plaintexts) are explicitly zeroed out immediately after use to minimize the window of exposure in the device's RAM.
