...
* **Best-Effort Memory Sanitization**: Application-managed sensitive byte buffers are explicitly overwritten with zeroes (`wipe()`) and sensitive key buffers are zeroized upon operation completion.
...
[ AES-256-GCM + AAD Validation ] (Decrypt mnemonic inside isolated scope)
       ↓
[ Derive Signing Key & Kaspa Schnorr Signing ] (Executed via audited TypeScript @noble/secp256k1 primitives)
       ↓
[ Signature Only Returned ]
       ↓
[ Zero Application Buffers & Release strict in-memory execution boundaries.
] (Sensitive byte buffers overwritten with zeroes; Sensitive byte buffers overwritten with zeroes
       ↓
...
* **Kaspa Core:** `@noble/secp256k1 and @noble/hashes`, `@kaspa/core-lib`

> **Note on `@noble/secp256k1 and @noble/hashes` Postinstall Patching:**
> `package.json` executes a `postinstall` script using `sed` to replace Node-specific `util` destructuring in `node_modules/@noble/secp256k1 and @noble/hashes/` with browser/mobile-native `globalThis.TextDecoder`/`TextEncoder`. Because string-substitution patches are fragile across upstream package releases or operating systems, developers should monitor `@noble/secp256k1 and @noble/hashes` releases for native mobile /web support and verify postinstall build compatibility when upgrading dependencies.
